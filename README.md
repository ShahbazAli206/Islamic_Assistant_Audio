# Islamic Assistant — Public Release Assets

Public downloads for the **Noor / Islamic Smart Assistant** desktop app.
**This repository contains release assets only — no source code.**

## Releases

<!-- DESKTOP-RELEASE:START -->
### `v1.3.18` — Desktop installer (latest)

[`ISMAA-Setup-1.3.18.exe`](https://github.com/ShahbazAli206/Islamic_Assistant_Audio/releases/download/v1.3.18/ISMAA-Setup-1.3.18.exe) — Windows installer for the Noor desktop app.
Published 2026-08-02. Installs from v1.2.0 onward auto-update to this version.
<!-- DESKTOP-RELEASE:END -->

### `audio-v1` — Offline translation audio
One ZIP per language of per-ayah Quran translation MP3s (`1.mp3` … `6236.mp3`,
global ayah numbering), generated with neural text-to-speech from authentic
published translations. The desktop app downloads these on demand and plays
them offline.

| | |
|--|--|
| Languages | 28 |
| Files per language | up to 6,236 (one per ayah) |
| Total size | ~10.3 GB |
| Naming | `releases/download/audio-v1/{lang}.zip` (e.g. `de.zip`) |
